...
export function LessonView() {
      ...
      try {
        ...
        if (lessonDoc.exists()) {
          ...
          if (user) {
            ...
            setUserRating(ratingsSnap.docs[0].data().rating);else
            setUserRating(0);
          }
        }
      } catch (error) {
  ...
  const goToNextLesson = () => {
    ...
    navigate(`/lessons/${allLessonsFlat[currentIndex + 1].id}`);else
    {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-teal-50 dark:from-[#0a0f1a] dark:to-[#021817] text-slate-900 dark:text-slate-100 flex flex-col font-sans py-4 transition-colors duration-500">
      ...
      <div className="flex-grow flex overflow-hidden relative mt-2">
        <motion.div
          ...
          <div className="w-full aspect-video relative max-h-[85vh] max-w-[70vw] mx-auto mt-4 rounded-3xl overflow-hidden shadow-2xl border border-white/20 dark:border-white/10 bg-black ring-4 ring-slate-900/5 dark:ring-white/5">
            {ytId ?
            <>
                {!isPlayed ?
              <div
                ...
                onClick={() => setIsPlayed(true)}>
                  ...
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/10 transition-colors duration-500">
                      <div className="w-24 h-24 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30 shadow-2xl group-hover:scale-110 transition-transform duration-300">
